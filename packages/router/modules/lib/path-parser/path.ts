...
export class Path<T extends Record<string, any> = Record<string, any>> {
  ...
  public build(params: T = {} as T, opts?: PathBuildOptions): string {
    ...
    const hasQueryTokens = this.tokens.some(t => /^query-parameter/.test(t.type))
    const base = this.tokens
      .filter(t => {
        if (/^query-parameter/.test(t.type)) return false
        // Remove single '?' from template if there are no query parameters in path
        if (!hasQueryTokens && t.type === 'delimiter' && t.match === '?') {
          return false
        }
        return true
      })
